...
#     contestant = get_object_or_404(Contestant, pk=contestant_id)
#     judge = Judge.objects.get(user=request.user)
#     scores = Score.objects.filter(judge=judge, contestant=contestant)

#     # criteria per category
#     criteria_by_category = {
#         'Fun': list(JudgingCriteria.objects.filter(category__name='Fun')),
#         'Function': list(JudgingCriteria.objects.filter(category__name='Function')),
#         'Engineering and crafting': list(JudgingCriteria.objects.filter(category__name='Engineering and crafting')),
#         'Creativity & Innovation': list(JudgingCriteria.objects.filter(category__name='Creativity & Innovation')),
#     }
...
#             for criterion in criteria_list:
#                 score_value = request.POST.get(f'criteria_{criterion.id}')
#                 score_object = scores.filter(criteria=criterion).first()
#             return render(request, 'scores/update_successful.html')
...
#                 if criterion == score_object.criteria:
#                     score_value = request.POST.get(f'criteria_{criterion.id}')
#                     score_object.score = score_value
#                     score_object.save()
#         # Handle score submission
#         return render(request, 'scores/submission_successful.html')
#     return render(request, 'scores/submit_score.html', {
